...
### Смена порта / Synology NAS

Хост-порт задаётся в `.env` рядом с `docker-compose.yml` (контейнер всегда слушает 80):

```env
HOST_IP=0.0.0.0
PORT=8180          # хост-порт: 8180 -> контейнер:80
```

На **Synology DSM** порт 80 занят самой DSM — поставьте свободный, например `8180` или `8080`.

**Через Synology Container Manager (GUI):**
1. Container Manager → Проекты → Создать → вставьте содержимое `docker-compose.yml` (порт пропишите прямо в `ports:`, напр. `"8180:80"`).
2. Образ `alexkuryshko/iptv-balance:latest` скачается автоматически из Docker Hub.
3. Папку для данных `/data` замонтируйте на Shared Folder (напр. `/volume1/docker/iptv-balance/data:/data`), чтобы конфиг и cookies кабинета пережили обновление контейнера.
4. Брандмауэр DSM / Firewall: разрешите входящий TCP по выбранному порту (Панель управления → Безопасность → Брандмауэр).
5. Откройте `http://<IP-NAS>:8180/`, войдите в кабинет, выберите плейлист, укажите адрес подключения вида `<IP-NAS>:8180` (или домен с портом) и протокол http/https — вставьте в плеер `http://<IP-NAS>:8180/plst.m3u8`.

> ARM-модели Synology (напр. на ARM Cortex, j-серии): нужен arm64-образ. CI собирает `linux/arm64` автоматически после добавления секретов `DOCKERHUB_USERNAME`/`DOCKERHUB_TOKEN` в GitHub (см. ниже) — до этого на Docker Hub доступен только `linux/amd64`.
